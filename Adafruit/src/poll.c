...
void main(void) {
    ...
    //uart_poll_out(uart_dev2, 0x0D);
    //uart_poll_out(uart_dev2, 0x0A);

    //uart_poll_out(uart_dev2, (uint8_t)0x44);
    ...
    //while (1) {
        ...
        // int ret = uart_poll_in(uart_dev2, &buffer[0]);
        // if (ret < 0) {
        ...
        for(int i=0; i<20; i++){
            ...
            ret = uart_poll_in(uart_dev2, &buf[i]);
